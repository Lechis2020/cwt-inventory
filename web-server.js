...
async function handleApi(req, res, pathname) {
  try {
    if (req.method === 'GET' && pathname === '/api/items') {
      sendJson(res, 200, { items: database.items });
      return;
    }
    ...
    if (req.method === 'POST' && pathname === '/api/items') {
      ...
      sendJson(res, 200, { success: true, item: result.item, items: database.items });
      return;
    }
    ...
    if (updateMatch && req.method === 'DELETE') {
      ...
      saveDatabase();
      sendJson(res, 200, { success: true, id: itemId, items: database.items });
      return;
    }
    ...
    if (adjustMatch && req.method === 'POST') {
      ...
      sendJson(res, 200, { success: true, id: result.item.id, quantity: result.item.quantity, items: database.items });
      return;
    }
